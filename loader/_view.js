...
if ((/##doCloseOnBlur##$/).test(location.href)) { // the view was an incognito panel (see below), so it should close it on blur
	global.addEventListener('blur', event => global.close());
	history.replaceState(null, '', location.href.replace(/##doCloseOnBlur##$/, ''));
} else
...
if (!main) { // in a Firefox incognito context without access to the background page
	...
	if (!tab) { // in a panel. Open a non-private mode popup where the panel would be
		...
		const { id: windowId, } = (await browser.windows.create({
			type: 'popup', url: location.href +'##doCloseOnBlur##', // the panel would close itself on blur, so emulate that (see above)
...
if (!main) { document.body.innerHTML = '<h1>500</h1>'; } // failed to move to non-private window
